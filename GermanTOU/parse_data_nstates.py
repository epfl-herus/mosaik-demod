...
from simulators.simulators import MultiSimulator, OccupancySimulator, TimedStatesSimulator, rescale_pdf, Simulator, monte_carlo_from_1d_cdf, monte_carlo_from_cdf, check_valid_cdf
from simulators.sparse_simulators import *
...
MAX_PERSONAS_HOUSEHOLD = 6 + 1 # always need to get one above the real max personas
# convert states 
...
personas_states = MAX_PERSONAS_HOUSEHOLD**primary_activities
...
personas_second_states = np.where(
    ...
    MAX_PERSONAS_HOUSEHOLD**np.where(sec_to_first[secondary_activities]==-1, 0, np.array(sec_to_first[secondary_activities], dtype=np.uint64)),
    0)
...
personas_states = np.where(
    ...
    MAX_PERSONAS_HOUSEHOLD**np.array(activity_offset + states_out_for_what - 1, dtype=np.uint64),
    personas_states
)
...
for act_number, activity_label in enumerate(activity_labels):
    n_doing_this_activity = states_to_plot   % (MAX_PERSONAS_HOUSEHOLD**(act_number+1)) // (MAX_PERSONAS_HOUSEHOLD**(act_number))
...
if np.array_equal(states_to_plot,hh_states):
    ...
    for act_number,  is_active_occ in enumerate(act_occ_mask):
        if is_active_occ:                
            active_occupants += hh_states   % (MAX_PERSONAS_HOUSEHOLD**(act_number+1)) // (MAX_PERSONAS_HOUSEHOLD**(act_number))
