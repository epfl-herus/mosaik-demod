...
def create_data_activity_profile(subgroup_kwargs, compiled_data_path = os.path.join('GermanTOU', 'compiled_data')):
    """Create the activity profiles of a day

    Args:
        subgroup_kwargs (dict): dictonary of the subgroup to generate the activity profile
        compiled_data_path (string, optional): The access path where the data should be saved. Defaults to os.path.join('GermanTOU', 'compiled_data').
    """
