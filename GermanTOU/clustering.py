...
from simulators.simulators import OccupancySimulator, TimedStatesSimulator, rescale_pdf
...
for split_time in times_splits:
    ...
    proportions = np.asarray([np.mean(inv[:,start_time:split_time] == i, axis=1) for i in range(len(possible_states_labels))]).T
    # concatenate in the data array
    X = np.c_[X, proportions]
    # concatenates the label of each feature of X
    names = np.r_[names, np.char.add(np.array(possible_states_labels, dtype=str), np.array(['_'+str(start_time/6) + '-'+str(split_time/6) for i in possible_states_labels]))]
    start_time = split_time
